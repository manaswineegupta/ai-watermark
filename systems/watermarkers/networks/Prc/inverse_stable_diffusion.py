        ...
        with torch.no_grad():
            ...
            for i, t in enumerate(self.progress_bar(timesteps_tensor)):
                ...
                if inv_order < 2 or (inv_order == 2 and i == 0):
                    ...
                    lambda_s, lambda_t = (
                        self.scheduler.lambda_t[s],
                        self.scheduler.lambda_t[t],
                    )
                    sigma_s, sigma_t = (
                        self.scheduler.sigma_t[s],
                        self.scheduler.sigma_t[t],
                    )
                    h = lambda_t - lambda_s
                    alpha_s, alpha_t = (
                        self.scheduler.alpha_t[s],
                        self.scheduler.alpha_t[t],
                    )
                    phi_1 = torch.expm1(-h)
                    ...
                    noise_pred = self.unet(
                        latent_model_input, s, encoder_hidden_states=text_embeddings
                    ).sample
                    # perform guidance
                    noise_pred = self.apply_guidance_scale(noise_pred, guidance_scale)
                    # convert noise to model output
                    model_s = self.scheduler.convert_model_output(
                        noise_pred, t, latents
                    )
                    x_t = latents

                    # Line 5
                    latents = (sigma_s / sigma_t) * (
                        latents + alpha_t * phi_1 * model_s
                    )
                    ...
                    if inverse_opt:
                        ...
                        if inv_order == 2 and i == 0:
                            latents = self.fixedpoint_correction(
                                latents,
                                s,
                                t,
                                x_t,
                    ...
                    with torch.no_grad():
                        ...
                        if i + 1 < len(timesteps_tensor):
                            ...
                            for tt in range(t, s, 10):
                                ss = tt + 10
                                lambda_s, lambda_t = (
                                    self.scheduler.lambda_t[ss],
                                    self.scheduler.lambda_t[tt],
                                )
                                sigma_s, sigma_t = (
                                    self.scheduler.sigma_t[ss],
                                    self.scheduler.sigma_t[tt],
                                )
                                h = lambda_t - lambda_s
                                alpha_s, alpha_t = (
                                    self.scheduler.alpha_t[ss],
                                    self.scheduler.alpha_t[tt],
                                )
                                phi_1 = torch.expm1(-h)
                                ...
                                noise_pred = self.unet(
                                    y_input, ss, encoder_hidden_states=text_embeddings
                                ).sample
                                noise_pred = self.apply_guidance_scale(
                            ...
                            y_t_model_input = self.scheduler.scale_model_input(
                                y_t_model_input, s
                            )
                            ...
                            model_s_output = self.scheduler.convert_model_output(
                                noise_pred, s, y_t
                            )
                            ...
                            y_model_input = self.scheduler.scale_model_input(
                                y_model_input, r
                            )

                            noise_pred = self.unet(
                                y_model_input, r, encoder_hidden_states=text_embeddings
                            ).sample
                            noise_pred = self.apply_guidance_scale(
                                noise_pred, guidance_scale
                            )
                            model_r_output = self.scheduler.convert_model_output(
                                noise_pred, r, y
                            )
                            ...
                            if inverse_opt:
                                latents = self.fixedpoint_correction(
                                    ...
                                    order=2,
                                    r=r,
                                    model_s_output=model_s_output,
                                    model_r_output=model_r_output,
                                    text_embeddings=text_embeddings,
                                    guidance_scale=guidance_scale,
                                    step_size=10 / t,
                                    scheduler=False,
        ...
        if order == 1:
            ...
            if scheduler:
                step_scheduler = StepScheduler(
                    current_lr=step_size, factor=factor, patience=patience
                )
            ...
            for i in range(n_iter):
                ...
                x_t_pred = (sigma_t / sigma_s) * input - (
                    alpha_t * phi_1
                ) * model_output
            ...
            lambda_r, lambda_s, lambda_t = (
                self.scheduler.lambda_t[r],
                self.scheduler.lambda_t[s],
                self.scheduler.lambda_t[t],
            )
            sigma_r, sigma_s, sigma_t = (
                self.scheduler.sigma_t[r],
                self.scheduler.sigma_t[s],
                self.scheduler.sigma_t[t],
            )
            alpha_s, alpha_t = self.scheduler.alpha_t[s], self.scheduler.alpha_t[t]
            ...
            for i in range(n_iter):
                ...
                if i == 0:
                    d = (1.0 / r0) * (model_s_output - model_r_output)
