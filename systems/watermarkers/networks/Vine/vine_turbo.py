...
class VINE_Turbo(torch.nn.Module, PyTorchModelHubMixin):
    def __init__(self, ckpt_path=None, device="cuda"):
        ...
        self.fixed_a2b_emb_base = text_encoder(
            fixed_a2b_tokens.unsqueeze(0).to(device)
        )[0].detach()
        ...
        self.timesteps = torch.tensor(
            [self.sched.config.num_train_timesteps - 1] * 1, device=device
        ).long()
    ...
    def load_ckpt_from_state_dict(self, ckpt_path, device):
        self.sec_encoder.load_state_dict(
            torch.load(os.path.join(ckpt_path, "ConditionAdaptor.pth"))
        )
        self.sec_encoder.to(device)
        self.sec_encoder.eval()

        self.unet.load_state_dict(
            torch.load(os.path.join(ckpt_path, "UNet2DConditionModel.pth"))
        )
        self.unet.to(device)
