...
def show_heatmap(img1, img2):
    ...
    cmaps = [
        "jet",
        mcolors.LinearSegmentedColormap.from_list("custom_cmap", ["black", "red"]),
    ]
    ...
    for cmap in cmaps:
        ...
        i += 1
        plt.subplot(len(cmaps), 3, i + 1)
        plt.imshow(np.clip(diff, 0, 1), cmap=cmap, alpha=1, vmin=0.0, vmax=0.3)
        plt.axis("off")
        i += 1
    plt.show()
