...
def activate_gradients(wm_gan, args):
    ...
    for i, param in enumerate(pbar):
        ...
        pbar.set_description(
            f"Watermarking {ctr}/{all_params_ctr} ({ctr/all_params_ctr*100:.2f}% parameters!"
        )
    return hooks
...
def save_tensor_as_pdf(x, file_name):
    grid = torchvision.utils.make_grid(
        x, nrow=1, range=(-1, 1), scale_each=True, normalize=True
    )
...
def plot_images(x: torch.Tensor, n_row=2, dpi=400, savefig=None, title=None):
    ...
    grid_img = torchvision.utils.make_grid(
        x, nrow=n_row, range=(-1, 1), scale_each=True, normalize=True
    )
    plt.axis("off")
    ...
    if savefig is not None and isdir(dirname(savefig)):
        ...
        print(
            f"Saved first img at '{os.path.abspath(first_img)}' figure at '{os.path.abspath(savefig)}'"
        )
