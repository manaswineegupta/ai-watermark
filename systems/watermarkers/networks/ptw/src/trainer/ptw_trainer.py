...
class PTWTrainer(Trainer):
    ...
    def save(self, ckpt: str):
        ...
        print(
            f"> Saved watermarked model to '{bcolors.OKGREEN}{os.path.abspath(ckpt)}{bcolors.ENDC}'."
        )
    ...
    def train(
        ...
        msg = WatermarkingKey.str_to_bits(wm_key.wm_key_args.message).unsqueeze(0)[
            :, : wm_key.wm_key_args.bitlen
        ]
        ...
        opt = Adam(
            generator.G.synthesis.parameters(),  # only the synthesis layers.
            lr=self.embed_args.ptw_lr,
            betas=(0 ** (4 / 5), 0.99 ** (4 / 5)),
        )

        accelerator = Accelerator()
        generator, reference_generator, opt = accelerator.prepare(
            generator, reference_generator, opt
        )
        ...
        with tqdm(
            total=self.env_args.log_every, desc="Training PTW Key (infinity loop)"
        ) as pbar:
            while True:  # infinity loop
                ...
                with torch.no_grad():
                    w_frozen, x_frozen = reference_generator.generate(
                ...
                x_train = torch.clamp(x_train, -1.0, 1.0)
                ...
                loss_lpips = (
                    self.embed_args.lambda_lpips * lpips_loss(x_train, x_frozen).mean()
                )
                ...
                if self.embed_args.lambda_id > 0:
                    loss_id = (
                        self.embed_args.lambda_id
                        * id_loss(x_train, torch.clamp(x_frozen, -1.0, 1.0))[0].mean()
                    )
                ...
                loss_dict["capacity"] = max(
                    ...
                    2.0
                    * (
                        bit_acc.avg * wm_key.wm_key_args.bitlen
                        - 0.5 * wm_key.wm_key_args.bitlen
                    ),
                )
                ...
                if step % self.env_args.log_every == 0:
                    ...
                    print(
                        f"> Save to: '{bcolors.OKGREEN}{self.embed_args.ckpt}{bcolors.ENDC}'"
                    )
                    print(f"----------------------------------------------------------")
                    print()
                ...
                if step % self.env_args.save_every == 0 and step > 0:
                    with torch.no_grad():
                        ...
                        msg_eval = (
                            msg[0]
                            .unsqueeze(0)
                            .repeat([self.env_args.eval_batch_size, 1])
                ...
                pbar.set_description(
                    f"Pivotal Tuning (infinity loop), Acc: {100*bit_acc.avg:.2f}%"
                )
