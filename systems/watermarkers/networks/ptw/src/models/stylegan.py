...
class StyleGAN(GAN):
        ...
        for name, layer in self.G.synthesis.named_children():
            ...
            for layer in layer.children():
                if "synthesis" in str(layer).lower() or "torgb" in str(layer).lower():
                    last_layer = layer
        self.post_processing_hooks.append(
    ...
    def sample_latents(self, batch_size: int = 32, truncation_psi: float = 1.0, c=None):
        seed = torch.randint(0, 2**32, (1,))
        z = np.random.RandomState(seed).randn(batch_size, self.G.z_dim)
        w = self.G.mapping(torch.from_numpy(z).to(self.device), c)[
            :, :1, :
        ]  # [1, 1, C]; fake w_avg
    ...
    def generate(
        self, batch_size=32, truncation_psi=1.0, w=None, expand=False, **kwargs
    ):
        """Generates random images."""
        if w is None:
            w = self.sample_latents(
                batch_size=batch_size, truncation_psi=truncation_psi
            )

        if expand:
            synth_images = self.G.synthesis(
                w.unsqueeze(1).repeat([1, self.G.mapping.num_ws, 1]), noise_mode="const"
            )
        else:
            synth_images = self.G.synthesis(w, noise_mode="const")
        # return w, torch.nn.Tanh()(synth_images)
    ...
    def save(self, **kwargs) -> dict:
        ...
        snapshot_data = dict(
            G=None, D=None, G_ema=self.G, augment_pipe=None, training_set_kwargs=dict()
        )
