...
from src.compatibility.stylegan_xl.torch_utils.ops import (
    conv2d_resample,
    upfirdn2d,
    bias_act,
    fma,
...
class SG2MessageSynthesisLayer(torch.nn.Module):
    ...
    def forward(self, x, w, noise_mode="random", fused_modconv=True, gain=1):
        ...
        for i, (x_i, style_i, msg_i) in enumerate(
            zip(x, styles, self.layer_mapper.get_msg())
        ):
            style_i = style_i + 0.01 * (
                modulation_dict["style"][i] if "style" in modulation_dict else 0
            )
            modulated_bias = self.bias + 0.01 * (
                modulation_dict["bias"][i] if "bias" in modulation_dict else 0
            )
            modulated_weight = self.weight * (
                1
                + 0.01
                * (modulation_dict["weight"][i] if "weight" in modulation_dict else 0)
            )
            ...
            r += [
                modulated_conv2d(
                    ...
                    styles=style_i.unsqueeze(0),
                    noise=noise if noise_mode == "const" else noise[i],
                    up=self.up,
                    padding=self.padding,
                    resample_filter=self.resample_filter.to(x.device),
                    flip_weight=flip_weight,
            ...
            r[-1] = bias_act.bias_act(
                r[-1].unsqueeze(0),
                modulated_bias.to(r[-1].dtype),
                act=self.activation,
                gain=act_gain,
                clamp=act_clamp,
            ).squeeze()
