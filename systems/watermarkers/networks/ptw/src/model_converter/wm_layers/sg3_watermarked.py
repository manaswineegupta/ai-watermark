...
class SG3MessageSynthesisLayer(torch.nn.Module):
    ...
    def forward(self, x, w, noise_mode="random", force_fp32=False, update_emas=False):
        ...
        for i, (x_i, style_i, msg_i) in enumerate(
            zip(x.to(dtype), styles, self.layer_mapper.get_msg())
        ):
            ...
            x_i = modulated_conv2d(
                x=x_i.unsqueeze(0).to(dtype),
                w=modulated_weight,
                s=style_i.unsqueeze(0),
            ...
            x_i = filtered_lrelu.filtered_lrelu(
                x=x_i,
                fu=self.up_filter,
                fd=self.down_filter,
        ...
        misc.assert_shape(
            new_x,
            [None, self.out_channels, int(self.out_size[1]), int(self.out_size[0])],
        )
