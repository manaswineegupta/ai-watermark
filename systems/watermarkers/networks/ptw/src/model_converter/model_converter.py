...
class ModelConverter:
    ...
    def detect_model_arch(self, model: GAN) -> str:
        if model.model_args.model_arch is not None:
            print(
                f"> Model architecture is {bcolors.OKGREEN}{model.model_args.model_arch}{bcolors.ENDC}."
            )
            return model.model_args.model_arch

        names = [name for name, layer in model.G.synthesis.named_children()]
        print(
            f"> No model arch provided. Resorting to guessing. Detected layer names: {names}"
        )
    ...
    def _build_sg2(self, model: GAN) -> Tuple[GAN, WMMapperGroup]:
        ...
        for layer_ctr, (name, child) in enumerate(
            tqdm(
                model.G.synthesis.named_children(),
                desc="Model Conversion",
            ...
            for gc_name, gc in child.named_children():
                ...
                mapper = LayerWMMapper(
                    bitlength=self.wm_key_args.bitlen, style_dim=gc.affine.out_features
                )  # instantiate the mapper.

                (
                    mapper.add_bias_mapper(gc.bias.shape)
                    if self.wm_key_args.bias_mapper
                    else None
                )
                (
                    mapper.add_style_mapper((gc.weight.shape[1],))
                    if self.wm_key_args.style_mapper
                    else None
                )
                (
                    mapper.add_weight_mapper((*gc.weight.shape[:2], 1, 1))
                    if self.wm_key_args.weight_mapper
                    else None
                )
                ...
                alternate_layer = SG2MessageSynthesisLayer(
                    in_channels=gc.weight.shape[1],
                    out_channels=gc.weight.shape[0],
                    w_dim=(
                        gc.w_dim if hasattr(gc, "w_dim") else gc._init_kwargs["w_dim"]
                    ),
                    up=gc.up,
                    use_noise=gc.use_noise,
                    activation=gc.activation,
                    kernel_size=gc.weight.shape[-1],
                    resolution=gc.resolution,
                    weight=deepcopy(gc.weight),
                    affine=deepcopy(gc.affine),
                    bias=deepcopy(gc.bias),
                    resample_filter=deepcopy(gc.resample_filter),
                    layer_mapper=mapper,
                )
    ...
    def _build_sg3(self, model: GAN) -> Tuple[GAN, WMMapperGroup]:
        ...
        for layer_ctr, (name, child) in enumerate(
            ...
            mapper = LayerWMMapper(
                bitlength=self.wm_key_args.bitlen, style_dim=child.affine.out_features
            )  # instantiate the mapper.

            (
                mapper.add_bias_mapper((*child.bias.shape,))
                if self.wm_key_args.bias_mapper
                else None
            )
            (
                mapper.add_style_mapper((child.affine.out_features,))
                if self.wm_key_args.style_mapper
                else None
            )
            (
                mapper.add_weight_mapper((*child.weight.shape[:2], 1, 1))
                if self.wm_key_args.weight_mapper
                else None
            )
            ...
            alternate_layer = SG3MessageSynthesisLayer(
                w_dim=child.w_dim,
