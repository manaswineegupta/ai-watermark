...
class LayerWMMapper(WMMapper):
    ...
    def summary(self):
        data = {
            "LayerWMMapper": {"bit_length": self.bitlength, "style_dim": self.style_dim}
        }
        if self.bias_head:
    ...
    def build(self):
        ...
        if self.backbone is None:
            ...
            )  # project the latent code + embedded_message
        ...
        if self.bias_shape:
            self.bias_head = torch.nn.Sequential(
                *[
                    torch.nn.Linear(
                        self.bitlength + self.style_dim, math.prod(self.bias_shape)
                    )
                ]
        ...
        if self.weight_shape:
            self.weight_head = torch.nn.Sequential(
                *[
                    torch.nn.Linear(
                        self.bitlength + self.style_dim, math.prod(self.weight_shape)
                    )
                ]
            ).to(self.env_args.device)
        if self.style_shape:
            self.style_head = torch.nn.Sequential(
                *[
                    torch.nn.Linear(
                        self.bitlength + self.style_dim, math.prod(self.style_shape)
                    )
                ]
            ).to(self.env_args.device)
    ...
    def forward(self, msg: torch.Tensor, style: torch.Tensor) -> dict:
        ...
        if self.bias_head:
            return_dict["bias"] = self.bias_head(torch.cat([msg, feats], dim=-1)).view(
                [-1, *self.bias_shape]
            )
        if self.weight_head:
            return_dict["weight"] = self.weight_head(
                torch.cat([msg, feats], dim=-1)
            ).view([-1, *self.weight_shape])
        if self.style_head:
            return_dict["style"] = self.style_head(
                torch.cat([msg, feats], dim=-1)
            ).view([-1, *self.style_shape])
        return return_dict
