...
from src.compatibility.stylegan_xl import dnnlib
...
class _LegacyUnpickler(pickle.Unpickler):
    def find_class(self, module, name):
        ...
        module = (
            module.replace("torch_utils", "src.compatibility.stylegan_xl.torch_utils")
            if "compatibility.stylegan_xl" not in module
            else module
        )
        module = (
            module.replace("dnnlib", "src.compatibility.stylegan_xl.dnnlib")
            if "compatibility.stylegan_xl" not in module
            else module
        )
        module = (
            module.replace("legacy", "src.compatibility.stylegan_xl.legacy")
            if "compatibility.stylegan_xl" not in module
            else module
        )
        module = (
            module.replace("pg_modules", "src.compatibility.stylegan_xl.pg_modules")
            if "compatibility.stylegan_xl" not in module
            else module
        )
        module = (
            module.replace(
                "feature_networks", "src.compatibility.stylegan_xl.feature_networks"
            )
            if "compatibility.stylegan_xl" not in module
            else module
        )
...
def convert_tf_generator(tf_G):
    ...
    # Convert kwargs.
    from src.compatibility.stylegan_xl.training import networks_stylegan2
