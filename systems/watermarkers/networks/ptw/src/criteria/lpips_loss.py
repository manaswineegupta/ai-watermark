...
class LPIPSLoss(nn.Module):
    def __init__(self):
        ...
        ckpt = torch.load("pretrained_models/vgg16-397923af.pth")
        vgg.load_state_dict(ckpt)
        self.vgg = vgg.features.eval().cuda()
        ###
    ...
    def preprocess(self, x):
        ...
        x = (
            x - torch.tensor(self.normalize.mean).view(1, 3, 1, 1).cuda()
        ) / torch.tensor(self.normalize.std).view(1, 3, 1, 1).cuda()
