...
class WatermarkingKeyArgs:
    ...
    def __post_init__(self):
        if self.message is None:
            self.message = "".join(
                np.random.choice(list(WatermarkingKeyArgs.ALPHABET), size=100)
            )

        if self.key_ckpt is not None and os.path.exists(
            self.key_ckpt
        ):  # attempt to load your own watermarking params
            data = torch.load(self.key_ckpt)
            key = self.key_ckpt
            print(
                f"> Restoring watermark arguments from '{bcolors.OKGREEN}{os.path.abspath(self.key_ckpt)}{bcolors.ENDC}'"
            )
            self.__dict__ = vars(data[self.WM_KEY_ARGS_KEY])
            self.key_ckpt = key

    bitlen: int = field(default=50, metadata={"help": "bit length of the watermark."})
    ...
    keygen_lambda_lpips: float = field(
        default=0.01, metadata={"help": "lambda for the lpips loss"}
    )

    keygen_lambda_id: float = field(
        default=0.01, metadata={"help": "lambda for the facial id loss"}
    )

    lr_mapper: float = field(
        default=0.001, metadata={"help": "learning rate for the mapper"}
    )

    lr_decoder: float = field(
        default=0.0001, metadata={"help": "learning rate for the detector"}
    )

    watermark_type: str = field(
        default=WatermarkType.PTW.value,
        metadata={
    ...
    weight_mapper: bool = field(
        default=False, metadata={"help": "whether to add a style mapper to the input. "}
    )

    bias_mapper: bool = field(
        default=False,
        metadata={"help": "whether to add a bias mapper to the deconvolution layers"},
    )

    style_mapper: bool = field(
        default=False, metadata={"help": "whether to add a style mapper to the input. "}
    )

    add_preprocessing: bool = field(
        default=False, metadata={"help": "add pre-processing to training"}
    )
