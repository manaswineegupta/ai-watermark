...
class ConfigArgs:
    ...
    def get_watermarking_key_args(self) -> WatermarkingKeyArgs:
        content: WatermarkingKeyArgs = self.args_to_config[
            WatermarkingKeyArgs.CONFIG_KEY
        ]
        return content
    ...
    def __post_init__(self):
        ...
        for i in range(
            2
        ):  # naive solution: Fill in keys, call post-init, fill in keys again
            keys_not_found = []
            ...
            if i == 0:
                if len(keys_not_found) > 0:
                    print_warning(
                        f"Could not find these keys: {keys_not_found}. Make sure they exist."
                    )
