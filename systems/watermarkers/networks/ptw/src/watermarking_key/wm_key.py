...
class WatermarkingKey(nn.Module):
    def __init__(self, wm_key_args: WatermarkingKeyArgs, env_args: EnvArgs = None):
        ...
        self.env_args = (
            EnvArgs() if env_args is None else env_args
        )  # assume default env arguments if none are given.
    ...
    def str_to_bits(msg: str) -> torch.Tensor:
        ...
        for i, letter in enumerate(msg):
            try:
                ...
                raise ValueError(
                    f"Letter '{letter}' is not in the alphabet ('{bcolors.OKGREEN}{WatermarkingKeyArgs.ALPHABET}{bcolors.ENDC}')."
                )
    ...
    def save(self, ckpt_fn: str = None) -> dict:
        ...
        save_dict = {WatermarkingKeyArgs.WM_KEY_ARGS_KEY: self.wm_key_args}
        if ckpt_fn is not None:
            print(
                f"> Saving Watermarking Decoder checkpoint to '{bcolors.OKGREEN}{os.path.abspath(ckpt_fn)}{bcolors.ENDC}'"
            )
    ...
    def validate(self, x: torch.Tensor, msg: Union[str, torch.Tensor]):
        ...
        bitwise_acc = (
            (msg_pred == msg[:, : self.wm_key_args.bitlen].to(x.device))
            .float()
            .mean(dim=1)
            .mean()
            .item()
    ...
    def loss(self, x: torch.Tensor, msg: torch.Tensor):
        ...
        return nn.BCEWithLogitsLoss()(
            extracted_msg[:, :bitlength], msg[:, :bitlength].to(extracted_msg.device)
        )
