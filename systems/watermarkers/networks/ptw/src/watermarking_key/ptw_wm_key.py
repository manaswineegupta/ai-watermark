...
class PTWWatermarkingKey(WatermarkingKey):
    MODEL_STATE_DICT_KEY = (
        "ptw_state_dict"  # the key in the dictionary to save the model state
    )
    ...
    def __init__(self, wm_key_args: WatermarkingKeyArgs, **kwargs):
        ...
        # )
        self.decoder = ModelFactory.load_decoder(
            wm_key_args.bitlen, decoder_arch=wm_key_args.decoder_arch
        )
    ...
    def load(self, ckpt_fn: str = None) -> None:
        ...
        # print(
        #    f"> Restored the state from {bcolors.OKGREEN}{os.path.abspath(ckpt_fn)}{bcolors.ENDC}. "
        # )

    def save(self, ckpt_fn: str = None) -> dict:
        """Create a single '*.pt' file. Returns only the dict if no ckpt_fn is provided."""
        save_dict = {self.MODEL_STATE_DICT_KEY: self.state_dict(), **super().save()}
        if ckpt_fn is not None:
            print(
                f"> Writing a PTW decoder to '{bcolors.OKGREEN}{os.path.abspath(ckpt_fn)}{bcolors.ENDC}'."
            )
    ...
    def prepare_generators_and_optimizers(self, g_target):
        ...
        assert (
            self.mappers.size() > 0
        ), "Need to specify at least one mapper to embed a watermark!"
            ...
            preprocessing = transforms.Compose(
                [
                    ...
                    transforms.RandomAffine(
                        degrees=(-5, 5), translate=(0.01, 0.01), scale=(0.95, 1.0)
                    ),
                ]
            )
        return preprocessing

    def learn(self, g_target: GAN, **kwargs):
        ...
        """
        assert (
            self.wm_key_args.key_ckpt is not None
        ), "Please specify a key_ckpt to save the key to."
        # print(self.env_args)

        self.initialize_training()
        run = self.setup_logging()
        g_frozen, opt_decoder, opt_mapper = self.prepare_generators_and_optimizers(
            g_target
        )
        ...
        g_frozen, g_target, self.mappers, opt_decoder, opt_mapper = accelerator.prepare(
            g_frozen, g_target, self.mappers, opt_decoder, opt_mapper
        )
        ...
        with tqdm(total=50, desc="Training PTW Key (infinity loop)") as pbar:
            while True:
                ...
                self.mappers.set_msg(
                    msg
                )  # forward the embedded_message to the mappers.
                ...
                if self.wm_key_args.keygen_lambda_lpips > 0:
                    loss_lpips = (
                        self.wm_key_args.keygen_lambda_lpips
                        * losses_funcs["lpips"](x, x_wm).mean()
                    )
                ...
                if self.wm_key_args.keygen_lambda_id > 0:
                    loss_id = (
                        self.wm_key_args.keygen_lambda_id
                        * losses_funcs["id"](x_wm, x)[0].mean()
                    )
                ...
                loss_dict["capacity"] = max(
                    0,
                    2
                    * (
                        (bit_acc.avg / 100) * self.wm_key_args.bitlen
                        - 0.5 * self.wm_key_args.bitlen
                    ),
                )
                ...
                if step % self.env_args.save_every == 0 and step > 0:
                    with torch.no_grad():
                        self.decoder.eval()
                        msg = self.gen_msg(n=self.env_args.eval_batch_size).to(
                            self.env_args.device
                        )
                        self.mappers.set_msg(
                            msg
                        )  # forward the embedded_message to the mappers
                        _, x = g_target.generate(
                ...
                pbar.set_description(
                    f"Training PTW Key (infinity loop, Acc: {bit_acc.avg:.2f}"
                )
