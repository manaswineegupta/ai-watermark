...
class ImageEmbeddingConditionedLatentDiffusion(LatentDiffusion):
        ...
        if self.training:
            c_adm = (
                torch.bernoulli(
                    ...
                    * torch.ones(c_adm.shape[0], device=c_adm.device)[:, None]
                )
                * c_adm
            )
    ...
    def log_images(self, batch, N=8, n_row=4, **kwargs):
        ...
        xc = log_txt_as_img(
            (x.shape[2], x.shape[3]), batch[self.cond_stage_key], size=x.shape[2] // 25
        )
        log["conditioning"] = xc
        uc = self.get_unconditional_conditioning(
            N, kwargs.get("unconditional_guidance_label", "")
        )
        unconditional_guidance_scale = kwargs.get("unconditional_guidance_scale", 5.0)
        ...
        with ema_scope(f"Sampling"):
            samples_cfg, _ = self.sample_log(
                ...
                batch_size=N,
                ddim=True,
                ddim_steps=kwargs.get("ddim_steps", 50),
