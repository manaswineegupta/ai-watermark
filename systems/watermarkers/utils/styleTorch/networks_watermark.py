...
def modulated_conv2d(
    ...
    modulated=True,  # modulate at all?
):
    ...
    if not modulated:
        return conv2d_resample.conv2d_resample(
            x=x,
            w=weight.to(x.dtype),
            f=resample_filter,
            up=up,
...
class SynthesisBlock(torch.nn.Module):
    def __init__(
        ...
        res_modulated_min_log2=2,  # modulated_conv2d_layer starts at res_modulated_min_log2
        res_modulated_max_log2=7,  # modulated_conv2d_layer ends at res_modulated_max_log2
        ...
        **layer_kwargs,  # Arguments for SynthesisLayer.
        ...
        effective_res = 2 if in_channels == 0 else int(np.log2(resolution))
        modulated = (
            effective_res >= res_modulated_min_log2
            and effective_res <= res_modulated_max_log2
        )

        if in_channels == 0:
            self.mid_channels = out_channels * self.resolution * self.resolution
            self.conv0 = FullyConnectedLayer(
                z_dim, self.mid_channels, activation="lrelu"
            )
        else:
    ...
    def forward(
        self, x, img, ws, z, force_fp32=False, fused_modconv=None, **layer_kwargs
    ):
        ...
        if self.in_channels == 0:
            next(w_iter)
            x = z.to(dtype=dtype, memory_format=memory_format)
        else:
            misc.assert_shape(
        ...
        if self.in_channels == 0:
            x = self.conv0(x).view(
                -1, self.out_channels, self.resolution, self.resolution
            )
            x = self.conv1(
                x,
                next(w_iter),
...
class SynthesisNetwork(torch.nn.Module):
    def __init__(
        self,
        z_dim,  # Latent atent (Z) dimensionality.
        w_dim,  # Intermediate latent (W) dimensionality.
        ...
        **block_kwargs,  # Arguments for SynthesisBlock.
        ...
        self.res_modulated_min_log2 = res_modulated_min_log2
        self.res_modulated_max_log2 = res_modulated_max_log2
        self.img_channels = img_channels
        ...
        for res in self.block_resolutions:
            ...
            block = SynthesisBlock(
                ...
                z_dim=z_dim,
                w_dim=w_dim,
                res_modulated_min_log2=res_modulated_min_log2,
                res_modulated_max_log2=res_modulated_max_log2,
                resolution=res,
    ...
    def forward(self, z, ws, **block_kwargs):
        block_ws = []
        ...
        for res, cur_ws in zip(self.block_resolutions, block_ws):
            block = getattr(self, f"b{res}")
            x, img = block(x, img, cur_ws, z, **block_kwargs)
        return img
...
class Generator(torch.nn.Module):
    ...
    def forward(
        self, z, w, c, truncation_psi=1, truncation_cutoff=None, **synthesis_kwargs
    ):
        ws = self.mapping(
            w, c, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff
        )
        img = self.synthesis(z, ws, **synthesis_kwargs)
        return img
...
class Decoder(torch.nn.Module):
    ...
    def forward(self, img, c, **block_kwargs):
        ...
        if self.c_dim > 0:
            cmap = self.mapping(None, c)
        x = self.b4(x, img, cmap)
        latents_out = x[:, : self.z_dim]
        watermark_logits_out = x[:, self.z_dim :]
        return latents_out, watermark_logits_out
