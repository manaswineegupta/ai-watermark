...
from special_loss.loss.color_wrapper import ColorWrapper, GreyscaleWrapper
from special_loss.loss.shift_wrapper import ShiftWrapper
from special_loss.loss.watson import WatsonDistance
from special_loss.loss.watson_fft import WatsonDistanceFft
from special_loss.loss.watson_vgg import WatsonDistanceVgg
from special_loss.loss.robust_loss import RobustLoss
from special_loss.loss.deep_loss import PNetLin
from special_loss.loss.ssim import SSIM
...
class LossProvider:
    ...
    def load_state_dict(self, filename):
        ...
        path = os.path.join("pretrained_models/loss_provider", "weights", filename)
    ...
    def get_loss_function(
        ...
        if model.lower() in ["l2"]:
            ...
            if is_greyscale:
                ...
                if pretrained:
                    loss.loss.load_state_dict(
                        map_weights(self.load_state_dict("gray_adaptive_trial0.pth"))
                    )
            else:
                loss = RobustLoss(
                    image_size=image_size,
                ...
                if pretrained:
                    loss.load_state_dict(
                        map_weights(self.load_state_dict("rgb_adaptive_trial0.pth"))
                    )
        else:
