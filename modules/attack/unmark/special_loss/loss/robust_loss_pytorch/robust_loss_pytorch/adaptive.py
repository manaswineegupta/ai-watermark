    ...
    def __init__(
        ...
        if alpha_lo == alpha_hi:
            ...
            self.fixed_alpha = torch.tensor(
                alpha_lo, dtype=self.float_dtype, device=self.device
            ...
            self.register_parameter(
                ...
                torch.nn.Parameter(
                    ...
                    .detach()
                    .to(dtype=self.float_dtype, device=self.device)[
        ...
        if scale_lo == scale_init:
            ...
            self.fixed_scale = torch.tensor(
                scale_init, dtype=self.float_dtype, device=self.device
            )[np.newaxis, np.newaxis].repeat(1, self.num_dims)
...
class StudentsTLossFunction(nn.Module):
    ...
    def __init__(self, num_dims, float_dtype, device, scale_lo=1e-5, scale_init=1.0):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if color_space not in color_spaces:
            raise ValueError(
                "`color_space` must be in {}, but is {!r}".format(
