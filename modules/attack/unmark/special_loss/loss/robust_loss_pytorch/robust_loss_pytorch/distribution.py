...
def analytical_base_partition_function(numer, denom):
    ...
    if not isinstance(numer, numbers.Integral):
        raise ValueError(
            "Expected `numer` of type int, but is of type {}".format(type(numer))
        )
    if not isinstance(denom, numbers.Integral):
        raise ValueError(
            "Expected `denom` of type int, but is of type {}".format(type(denom))
        )
    if not numer >= 0:
    ...
    b_q = ((np.arange(-0.5, numer - 0.5, dtype=np.float64)) / numer).tolist() + (
        np.arange(1, 2 * denom, dtype=np.float64) / (2 * denom)
    ).tolist()
    z = (1.0 / numer - 1.0 / (2 * denom)) ** (2 * denom)
    mult = (
        np.exp(np.abs(2 * denom / numer - 1.0))
...
def partition_spline_curve(alpha):
    ...
    x = torch.where(
        ...
        5.0 / 18.0 * util.log_safe(4 * alpha - 15) + 8,
    )
    return x
