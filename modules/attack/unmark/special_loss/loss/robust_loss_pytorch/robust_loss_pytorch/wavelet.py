...
def generate_filters(wavelet_type=None):
    ...
    supported_half_filters = {
        ...
        "CDF9/7": HalfFilters(
            ...
            hi=np.array(
                [+0.788485616406, -0.418092273222, -0.040689417609, +0.064538882629]
            ),
        ),
        # Le Gall 5/3 filters (sometimes called CDF 5/3 filters).
        "LeGall5/3": HalfFilters(
            lo=np.array([0.75, 0.25, -0.125]) * np.sqrt(2.0),
            hi=np.array([1.0, -0.5]) / np.sqrt(2.0),
        ),
    }  # pyformat: disable
...
def pad_reflecting(x, padding_below, padding_above, axis):
    ...
    if not isinstance(axis, int):
        raise ValueError(
            "Expected `axis` of type int, but is of type {}".format(type(axis))
        )
    if not (axis >= 0 and axis < len(x.shape)):
        raise ValueError(
            "Expected `axis` in [0, {}], but is = {}".format(len(x.shape) - 1, axis)
        )
    x = torch.as_tensor(x)
    ...
    y = torch.index_select(x, axis, torch.as_tensor(j).type(torch.int64).to(x.device))
    return y
...
def _check_resample_inputs(x, f, direction, shift):
    ...
    if len(x.shape) != 3:
        raise ValueError(
            "Expected `x` to have rank 3, but is of size {}".format(x.shape)
        )
    if len(f.shape) != 1:
        raise ValueError(
            "Expected `f` to have rank 1, but is of size {}".format(f.shape)
        )
...
def _upsample(x, up_sz, f, direction, shift):
    ...
    x_undecimated = torch.reshape(torch.stack(x_and_zeros, direction + 2), list(sz_ex))
    # Ensure that `x_undecimated` has a size of `up_sz`, by slicing and padding
    # as needed.
    x_undecimated = x_undecimated[:, 0 : up_sz[0], 0 : up_sz[1]]
    x_undecimated = torch.nn.functional.pad(
        x_undecimated,
        [
            0,
            up_sz[1] - x_undecimated.shape[2],
            0,
            up_sz[0] - x_undecimated.shape[1],
            0,
            0,
        ],
    )
    ...
    x_padded = pad_reflecting(
        x_undecimated, len(f) // 2, (len(f) - 1) // 2, direction + 1
    )
    y = torch.conv2d(
        x_padded[:, np.newaxis, :, :],
        f_ex[np.newaxis, np.newaxis].type(x.dtype),
...
def collapse(pyr, wavelet_type):
    ...
    if not isinstance(pyr, (list, tuple)):
        raise ValueError(
            "Expected `pyr` to be a list or tuple, but is a {}".format(type(pyr))
        )
    ...
    for d in range(len(pyr) - 2, -1, -1):
        flat = torch.cat(
            [
                ...
                torch.cat([pyr[d][2], pyr[d][0]], dim=2),
            ],
            dim=1,
        )  # pyformat: disable
    return flat
...
def visualize(pyr, percentile=99.0):
    ...
    for d in range(len(pyr) - 1):
        ...
        for b in range(3):
            ...
            max_mag = vec[
                np.int64(np.floor(vec.shape[0] * np.clip(percentile / 100.0, 0.0, 1.0)))
            ]
            vis_band.append(0.5 * (1.0 + torch.clamp(band / max_mag, -1.0, 1.0)))
        vis_pyr.append(vis_band)
    d = len(pyr) - 1
    resid = torch.as_tensor(pyr[d])
    resid_norm = (resid - torch.min(resid)) / (torch.max(resid) - torch.min(resid))
    vis_pyr.append(resid_norm)
    vis = torch.round(255.0 * flatten(vis_pyr).permute([1, 2, 0])).type(torch.uint8)
    return vis
