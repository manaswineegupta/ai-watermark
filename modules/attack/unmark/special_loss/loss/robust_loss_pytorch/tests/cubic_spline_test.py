...
class TestCubicSpline(parameterized.TestCase):
    ...
    def testZeroTangentMidpointValuesAndDerivativesAreCorrect(
        ...
        y_true = (values[0:-1] + values[1:]) / 2.0
    ...
    def testExtrapolationIsLinear(self, float_dtype, device):
        ...
        x_above = float_dtype((np.random.uniform(size=1024) + 1.0) * (n - 1))
        for _ in range(8):
            values = float_dtype(np.random.normal(size=n))
            tangents = float_dtype(np.random.normal(size=n))
            ...
            y_below = self._interpolate1d(
                x_below, values, tangents, float_dtype, device
            )[0]
            ...
            y_above = self._interpolate1d(
                x_above, values, tangents, float_dtype, device
            )[0]
