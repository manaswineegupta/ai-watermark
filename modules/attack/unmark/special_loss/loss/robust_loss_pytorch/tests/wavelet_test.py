...
class TestWavelet(parameterized.TestCase):
    ...
    def testPadWithOneReflectionIsCorrect(self):
        ...
        for _ in range(4):
            ...
            axis = int(np.floor(np.random.uniform() * 3.0))

            if axis == 0:
                reference = np.pad(
                    x, [[padding_below, padding_above], [0, 0], [0, 0]], "reflect"
                )
            elif axis == 1:
                reference = np.pad(
                    x, [[0, 0], [padding_below, padding_above], [0, 0]], "reflect"
                )
            elif axis == 2:
                reference = np.pad(
                    x, [[0, 0], [0, 0], [padding_below, padding_above]], "reflect"
                )
    ...
    def testAnalysisLowpassFiltersAreNormalized(self):
        ...
        for wavelet_type in wavelet.generate_filters():
            ...
            magnitude = np.sum(
                filters.analysis_lo[:, np.newaxis] * filters.analysis_lo[np.newaxis, :]
            )
            np.testing.assert_allclose(magnitude, 2.0, atol=1e-10, rtol=1e-10)
    ...
    def testWaveletTransformationIsVolumePreserving(self):
        ...
        for wavelet_type in wavelet.generate_filters():
            ...
            np.testing.assert_allclose(det, 1.0, atol=1e-5, rtol=1e-5)
    ...
    def testVisualizeMatchesGoldenData(self):
        ...
        np.testing.assert_allclose(
            np.float32(vis_true) / 255.0,
            np.float32(vis) / 255.0,
            atol=0.005,
            rtol=0.005,
        )
        ...
        for wavelet_type in wavelet.generate_filters():
            ...
            if d == (len(pyr) - 1):
                if np.prod(pyr[d].shape) > 0:
                    c, i, j = np.int32(
                        np.floor(np.array(np.random.uniform(size=3)) * pyr[d].shape)
                    ).tolist()
                    pyr[d][c, i, j] = v
            else:
                ...
                if np.prod(pyr[d][b].shape) > 0:
                    c, i, j = np.int32(
                        np.floor(np.array(np.random.uniform(size=3)) * pyr[d][b].shape)
                    ).tolist()
                    pyr[d][b][c, i, j] = v
    ...
    def testRescaleAndUnrescaleReproducesInput(self):
        ...
        pyr_recon = wavelet.rescale(pyr_rescaled, 1.0 / scale_base)
