...
def _generate_pixel_toy_image_data(image_width, num_samples, _):
    ...
    samples = np.random.normal(loc=np.tile(mu[np.newaxis], [num_samples, 1, 1, 1]))
    reference = np.mean(samples, 0)
    return samples, reference, color_space, representation


def _generate_wavelet_toy_image_data(image_width, num_samples, wavelet_num_levels):
    ...
    mu = scaling * np.random.uniform(size=(3, w, w))
    band_samples = np.random.normal(loc=np.tile(mu[np.newaxis], [num_samples, 1, 1, 1]))
    band_reference = np.mean(band_samples, 0)
    ...
    samples = np.transpose(
        np.reshape(samples, [num_samples, 3, image_width, image_width]), [0, 2, 3, 1]
    )
...
class TestAdaptive(parameterized.TestCase):
    ...
    def testInitialAlphaAndScaleAreCorrect(self, float_dtype, device_string):
        ...
        for i in range(8):
            ...
            alpha_hi = float_dtype(np.random.uniform() + 1.0)
            # Half of the time pick a random initialization for alpha, the other half
            # use the default value.
            if i % 2 == 0:
                alpha_init = float_dtype(
                    alpha_lo + np.random.uniform() * (alpha_hi - alpha_lo)
                )
                true_alpha_init = alpha_init
            else:
                alpha_init = None
                true_alpha_init = (alpha_lo + alpha_hi) / 2.0
    ...
    def _sample_cauchy_ppf(self, num_samples):
        ...
        p = np.arange(0.0, 1.0, spacing) + spacing / 2.0
        return scipy.stats.cauchy(0.0, np.sqrt(2.0)).ppf(p)
    ...
    def _sample_normal_ppf(self, num_samples):
        ...
        spacing = 1.0 / num_samples
        p = np.arange(0.0, 1.0, spacing) + spacing / 2.0
        return scipy.stats.norm(0.0, 1.0).ppf(p)
    ...
    def _sample_nd_mixed_data(self, n, m, float_dtype):
        ...
        scale = np.exp(np.clip(np.random.normal(size=m), -3.0, 3.0))
        samples = (
            np.tile(samples0[:, np.newaxis], [1, m]) * (alpha[np.newaxis, :] == 0.0)
            + np.tile(samples2[:, np.newaxis], [1, m]) * (alpha[np.newaxis, :] == 2.0)
        ) * scale[np.newaxis, :] + mu[np.newaxis, :]
    ...
    def testFittingToyNdMixedDataIsCorrect(self, float_dtype, device_string):
        ...
        mu = Variable(
            torch.tensor(np.zeros(samples.shape[1], dtype=float_dtype), device=device),
            requires_grad=True,
        )

        adaptive_lossfun = adaptive.AdaptiveLossFunction(num_dims, float_dtype, device)
        params = torch.nn.ParameterList(adaptive_lossfun.parameters())
    ...
    def testFittingToyNdMixedDataIsCorrectStudentsT(self, float_dtype, device_string):
        ...
        students_t_lossfun = adaptive.StudentsTLossFunction(
        ...
        for ldf, a_true in zip(log_df, alpha_true):
            if a_true == 0:
                np.testing.assert_allclose(ldf, 0.0, rtol=0.1, atol=0.1)
            elif a_true == 2:
                np.testing.assert_(np.all(ldf > 4))
        scale /= np.sqrt(2.0 - (alpha_true / 2.0))
    ...
    def testLossfunPreservesDtype(self, float_dtype, device_string):
        ...
        adaptive_lossfun = adaptive.AdaptiveLossFunction(num_dims, float_dtype, device)
        loss = (
            adaptive_lossfun.lossfun(torch.tensor(samples, device=device))
            .cpu()
            .detach()
            .numpy()
        )
    ...
    def testImageLossfunPreservesDtype(self, float_dtype, device_string):
        ...
        loss = (
            adaptive_image_lossfun.lossfun(torch.tensor(x, device=device))
            .cpu()
            .detach()
            .numpy()
        )
